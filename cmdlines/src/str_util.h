...
#ifndef NEWPLAN_TOOLKIT_CORE_PLATFORM_STR_UTIL_H_
#define NEWPLAN_TOOLKIT_CORE_PLATFORM_STR_UTIL_H_
...
namespace newplan_toolkit
{
    namespace str_util
    {
        ...
        {
            return text.empty() ? std::vector<string>() : absl::StrSplit(text, absl::ByAnyChar(delims));
        }

        template <typename Predicate>
        std::vector<string> Split(StringPiece text, StringPiece delims, Predicate p)
        {
            return text.empty() ? std::vector<string>() : absl::StrSplit(text, absl::ByAnyChar(delims), p);
        }
...
#endif // NEWPLAN_TOOLKIT_CORE_PLATFORM_STR_UTIL_H_
