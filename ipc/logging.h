...
public:
    ...
    get_tracer_instance(std::string func, std::string src_file,
                        size_t tracing_line, std::string info, size_t thread_id)
    {
        static std::mutex tracer_protector;

        std::unique_ptr<Tracer> tracer_data(
            new Tracer(func, src_file, tracing_line, info, thread_id,
                       Tracer::get_struct_log(), tracer_protector));

        return (tracer_data);
    }

protected:
    explicit Tracer(std::string function_name, std::string file_name,
                    size_t tracing_line, std::string info, size_t thread_id,
                    LogStructure &global_log_map,
                    std::mutex &global_tracer_protector)
        : log_map_(global_log_map), tracer_protector(global_tracer_protector),
          tracing_function_(function_name), tracing_file_(file_name),
          tracing_line_(tracing_line), tracing_info_(info),
          tracing_tid_(thread_id)
    {
#ifndef LOG_TERMINAL
        auto tuple_value = std::tuple(tracing_function_, tracing_file_,
                                      tracing_line_, tracing_info_, 1);
        {
...
#else
        VLOG(TRACING_LOG_LEVEL)
            << "[TRACING-IN]: [" << tracing_function_ << " @ "
            << tracing_file_.substr(tracing_file_.find_last_of("/") + 1) << ":"
            << tracing_line_ << "]    " << tracing_info_;
#endif
    }
...
#define TRACING_UNIQUE(info, count)                                      \
    auto CONNECT(value_, count) = std::move(Tracer::get_tracer_instance( \
        __FUNCTION__, __FILE__, __LINE__, (info), get_tid()))

#define UNIMPLEMENTED                                  \
    LOG(FATAL) << "Function \"" << __PRETTY_FUNCTION__ \
...
#define TRACE_OUT \
    VLOG(5) << "[TRACING] out function \"" << __FUNCTION__ << "\"";
