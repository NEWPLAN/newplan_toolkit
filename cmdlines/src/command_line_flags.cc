...
namespace tensorflow
{
    namespace
    {
                             ...
                             bool *value_parsing_ok)
        {
            *value_parsing_ok = true;
            if (absl::ConsumePrefix(&arg, "--") && absl::ConsumePrefix(&arg, flag) && absl::ConsumePrefix(&arg, "="))
            {
    ...
    Flag::Flag(const char *name, tensorflow::int32 *dst, const string &usage_text,
        ...
        int32_default_for_display_(*dst), usage_text_(usage_text)
    {
    }
    ...
    Flag::Flag(const char *name, tensorflow::int64 *dst, const string &usage_text,
        ...
        int64_default_for_display_(*dst), usage_text_(usage_text)
    {
    }
    ...
    Flag::Flag(const char *name, float *dst, const string &usage_text,
        ...
        float_default_for_display_(*dst), usage_text_(usage_text)
    {
    }
    ...
    Flag::Flag(const char *name, bool *dst, const string &usage_text,
        ...
        bool_default_for_display_(*dst), usage_text_(usage_text)
    {
    }
    ...
    Flag::Flag(const char *name, string *dst, const string &usage_text,
        ...
        string_default_for_display_(*dst), usage_text_(usage_text)
    {
    }
    ...
    {
        // VLOG(0) << type_ << " for " << arg << ", name = " << name_;
        bool result = false;
        if (type_ == TYPE_INT32)
        {
        ...
        {
            VLOG(0) << "UNKNOWN TYPE for " << arg << ", the type is: " << type_;
        }
        return result;
    }
