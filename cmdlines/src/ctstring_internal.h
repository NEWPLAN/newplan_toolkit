...
#ifndef NEWPLAN_TOOLKIT_CORE_PLATFORM_CTSTRING_INTERNAL_H_
#define NEWPLAN_TOOLKIT_CORE_PLATFORM_CTSTRING_INTERNAL_H_
...
#if (defined(__BYTE_ORDER__) && defined(__ORDER_LITTLE_ENDIAN__) && __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__) || defined(_WIN32)
...
static inline size_t TF_align16(size_t i)
{
    return (i + 0xF) & ~0xF;
}
...
enum
{
    TF_TString_SmallCapacity =
        (sizeof(TF_TString_Union) - sizeof(/* null delim */ char) - sizeof(/* uint8_t size */ uint8_t)),
};
                                                ...
                                                TF_TString_Type type)
{
    ...
    return (mask & (size << 2)) | (~mask & size) | ((size_t)type << ((sizeof(size_t) - 1) * 8)); // NOLINT
#endif // TF_TSTRING_LITTLE_ENDIAN
}
...
{
    if (TF_TString_GetType(str) == TF_TSTR_LARGE && str->u.large.ptr != NULL)
    { // NOLINT
...
#endif // NEWPLAN_TOOLKIT_CORE_PLATFORM_CTSTRING_INTERNAL_H_
