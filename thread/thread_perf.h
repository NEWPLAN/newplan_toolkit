...
#define CK_TIME 1
    ...
    static int EnumFile(vector<FileAttribute> &file_array, string _dir)
    {
    ...
    static int get_sys_mem(char *mem)
    {
    ...
    static int get_phy_mem(pid_t pid, char *ph)
    {
        char file[64] = {0};       //文件名
        FILE *fd;                  //定义文件指针fd
    ...
    static int get_process_time(pid_t pid, int tid)
    {
        char szStatStr[1024];
        char pname[64];
        char state;
    ...
    */
    ...
    static string GetCpuMem(size_t pid, string &cpu, string &mem, int tid = 0)
    {
        FILE *fp;
        char buf[128];
        char tcpu[7];
