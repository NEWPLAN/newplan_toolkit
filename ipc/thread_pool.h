...
#ifndef __NP_NET_CORE_THREAD_POOL_H__
#define __NP_NET_CORE_THREAD_POOL_H__
    ...
    public:
                         ...
                         const size_t num_blocks = 0 BS_THREAD_POOL_PRIORITY_INPUT)
        {
          ...
          for (T i = start; i < end; ++i)
            loop(i); } BS_THREAD_POOL_PRIORITY_OUTPUT);
            }
        }
                             ...
                             F &&sequence BS_THREAD_POOL_PRIORITY_INPUT)
        {
            for (T i = first_index; i < index_after_last; ++i)
                detach_task([sequence = std::forward<F>(sequence), i]
                            { sequence(i); } BS_THREAD_POOL_PRIORITY_OUTPUT);
        }
            ...
            if (index_after_last > first_index)
            {
                ...
                for (T i = first_index; i < index_after_last; ++i)
                    future.push_back(submit_task([sequence = std::forward<F>(sequence), i]
                                                 { return sequence(i); } BS_THREAD_POOL_PRIORITY_OUTPUT));
                return future;
            }
            return {};
        }
